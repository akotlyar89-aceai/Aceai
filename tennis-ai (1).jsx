...
function StrategyChat({ systemPrompt, quickPrompts, placeholder, onGamePlan }) {
  ...
  async function sendMessage(text) {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: ANTHROPIC_MODEL, max_tokens: 1000, system: systemPrompt, messages: newMessages }),
      });
      if (res.ok) {
        const data = await res.json();
        reply = data.content?.map(b => b.text || "").join("").trim() || null;
    ...
    if (!reply) reply = getFallbackReply(userText, systemPrompt);

    // Parse the reply into a game plan and push to visual
    const plan = parseGamePlan(reply, userText);
    if (plan) onGamePlan(plan);
  ...
  function getFallbackReply(question) {
    const q = question.toLowerCase();
    if (q.includes("pusher") || q.includes("retriever")) return "vs a pusher: 1. Draw them forward with a short slice, then pass deep. 2. Hit high topspin to their backhand corner — force them behind the baseline. 3. Be patient — wait for the short ball before attacking. 4. Move them side to side to open the court. 5. Attack when they're inside the service box. Avoid: going for too much too early. Key: 'Deep and patient — make them earn every point.'";
    if (q.includes("big hitter") || q.includes("flat baller")) return "vs a big hitter: 1. Block and redirect their pace rather than swinging big. 2. Hit heavy topspin above their strike zone — they hate high balls. 3. Slice approach down the middle to neutralize their angles. 4. Moonball to reset when under pressure. 5. Take pace off — slow balls disrupt their timing. Avoid: trading flat groundstrokes. Key: 'Change pace, change spin — take away their rhythm.'";
    if (q.includes("net") || q.includes("volley")) return "vs a net rusher: 1. Topspin lob over their backhand shoulder — their worst nightmare. 2. Drive low at their feet to force the difficult volley. 3. Go crosscourt away from their approach direction. 4. Step in early and take the ball on the rise for a sharp pass. 5. Mix up lobs and passes to keep them guessing. Avoid: going for the perfect pass every time. Key: 'Lob early, pass late — keep them honest.'";
    return "Game plan: 1. Control the rally with heavy crosscourt topspin. 2. Attack every short ball down the line. 3. Serve wide on deuce, T on ad to create angles. 4. Come to net behind a strong approach. 5. Stay patient — let the pattern develop. Avoid: going for too much from behind the baseline. Key: 'Build the point — don't skip steps.'";
  }
  ...
  return (
    <div className="ai-chat-box">
      ...
      <div className="chat-messages">
        {messages.length === 0 && (
          <div className="msg ai">👋 Describe your opponent and situation — I'll build you a game plan and update the tactical breakdown on the left in real time.</div>
        )}
        {messages.map((m, i) => (
      ...
      <section className="ai-section" id="coach">
        ...
        <div className={`ai-panel ${activeTab === "strategy" ? "active" : ""}`}>
          <div className="ai-panel-left">
            ...
            <p>Tell ACE about your opponent and the surface — get a complete tactical game plan. Your top patterns update live in the panel below as you chat.</p>
